...
def _mock_ocr(
    operation_location_header: str, mock_operation_id: str, result: dict
) -> None:
    respx.post(
...
async def test_first_analysis_is_persisted_and_second_uses_cache(
    ...
    excel = await async_client.get(
        f"/api/v1/financial-summary/{first.json()['document_id']}/excel"
    )
...
async def test_extraction_failure_is_recorded_without_document_content(
    ...
    async def fail_extract(_self: FinancialSummaryExtractor, _regions: object) -> list[ExtractedField]:
